...
static unsigned long delaypublish;

void MQTT_connect();
void tankcallback();
void irrigationcallback();
...
void showParsedData() {
  ...
  if ( (millis() - delaypublish) <= 60000) {
    ph.publish(phValue);
  }

  if ( ((millis() - delaypublish) >= 60000) && ((millis() - delaypublish) < 120000) ) {
    ec.publish(ecValue);
  }

  if ( ((millis() - delaypublish) >= 120000) && ((millis() - delaypublish) < 1800000) ) {
    temp.publish(tempValue);
  }

  if ( (millis() - delaypublish) >= 1900000) {
    delaypublish = millis();
  }
